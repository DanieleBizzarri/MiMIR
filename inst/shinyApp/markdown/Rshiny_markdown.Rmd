...
plot.na.heatmap(t(metabo_measures[,MET57]))
```

### Correlation of the metabolites 
```{r corr metabolites, echo=FALSE}
  res<-cor.assoc(metabo_measures,metabo_measures, MET57,MET57)
  plot.corply(res, main="Metabolites' Correlations", reorder.x=TRUE, abs=F, 
                      resort_on_p= TRUE,reorder_dend=F)
```

### Histogram metabolites
```{r histograms metabolites, echo=FALSE, out.width="150%", out.height="600%"}
#metabo_barplots(metabo_measures = metabo_measures[,MET57], color="red")
one_hist(metabo_measures[,MET57])
```
...
```{r corr pheno, echo=FALSE}
  res<-cor.assoc(data.matrix(bin_phenotypes),data.matrix(bin_phenotypes), bin_pheno_available, bin_pheno_available)
  plot.corply(res, main="Binarized phenotypes correlations", reorder.x=TRUE, abs=F, 
                      resort_on_p= TRUE,reorder_dend=F)
```
...
plot.corply(res, main="Correlations of the Metabolic scores", reorder.x=TRUE, abs=F,
                    resort_on_p= TRUE,reorder_dend=F)
```
...
htmltools::tagList(lapply(bin_pheno_available, function(i){
    ...
    c1<-plattCalib_plot(calibrations[[i]],name=paste0("s_",i), nbins = Nbins, annot_x=c(1.22,1.22), annot_y=c(-0.9,-0.6))
...
```{r calibrations corr, echo=FALSE}
  ...
  res<-cor.assoc(calib,calib, colnames(calib),colnames(calib))
  plot.corply(res, main="Calibrated surrogates' correlations", reorder.x=TRUE, abs=F,
                      resort_on_p= TRUE,reorder_dend=F)
```
